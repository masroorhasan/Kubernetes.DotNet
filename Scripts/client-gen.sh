...
if [ -f "${CURRENT_DIR}/Kubernetes/src/${KUBERNETES_SRC_DIR}" ]; then
    swagger-codegen generate -i ${CURRENT_DIR}/Swagger/swagger.json -l csharp -o ${CURRENT_DIR}/Kubernetes -c ${CURRENT_DIR}/Scripts/update-config.json
else
    swagger-codegen generate -i ${CURRENT_DIR}/Swagger/swagger.json -l csharp -o ${CURRENT_DIR}/Kubernetes -c ${CURRENT_DIR}/Scripts/init-config.json
fi
